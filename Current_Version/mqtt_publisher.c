...
void Send_Data() {
    char publish[128];
    char topic[20];
    int temperature=0, humidity=0, gas=0, room=0;
    sqlite3* db;
    sqlite3_stmt* stmt;
    ...
    if (check==0) { 
        ...
        while(sqlite3_step(stmt) != SQLITE_DONE){ 
            ...
            room = sqlite3_column_int(stmt,4);
            ...
            printf("Data sent temp for topic %d->temp:%d hum:%d gas:%d\n",data[last_check_room].last_room,data[last_check_room].last_temperature,data[last_check_room].last_humidity,data[last_check_room].last_gas);
            sprintf(publish,"%d %d %d",data[last_check_room].last_temperature,data[last_check_room].last_humidity,data[last_check_room].last_gas);
        ...
        for(int i = 0;i<last_check_room;i++){ 
            printf("Data sent temp for topic %d->temp:%d hum:%d gas:%d\n",data[i].last_room,data[i].last_temperature,data[i].last_humidity,data[i].last_gas);
